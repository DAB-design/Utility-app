...
with col1:
    ...
    if img_file:
        ...
        if st.button("🔍 Scan for Pole Tag"):
    ...
    with st.form("audit_form"):
        ...
        if measurement > 0:
            if measurement < req_limit:
                st.error(f"❌ FAIL: {measurement} {unit} is insufficient.")
                result = "FAIL"
            else:
                st.success(f"✅ PASS: Compliant with {rule_data['ref']}.")
                result = "PASS"
        
        st.divider()
        
        # VISUAL ISSUES CHECKLIST
        st.markdown("### ⚠️ Visual Violations")
