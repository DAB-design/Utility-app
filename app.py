...
st.sidebar.header("🛠️ Compliance Toolkit")
...
if tool_mode == "NESC Compliance Check":
    ...
    if rule == "Comm Worker Safety Zone (40in)":
        ...
        if measured > 0:
            if measured < 40:
                st.sidebar.error(f"❌ VIOLATION: {measured}in is unsafe.")
            else:
...
with col1:
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 OCR Scan (Read Text)"):
            try:
                # Basic OCR attempt
...
# 3. DATA PREVIEW
st.divider()
st.subheader("📂 Project Data Sheet")
if os.path.exists(DATA_FILE):
